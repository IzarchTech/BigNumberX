...
namespace BigIntegerX.Test
{
    ...
    /// </remarks>
    [TestFixture]
    public class DecimalXTest
    {
        #region test parsing support
        public void GetThreeArgs(string test, out string arg1, out string arg2, out string result)
        {
            var atoms = test.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            arg1 = StripSingleQuotes(atoms[2]);
            arg2 = StripSingleQuotes(atoms[3]);
            result = StripSingleQuotes(atoms[5]);
        }

        public void GetTwoArgs(string test, out string arg, out string result)
        {
            var atoms = test.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public string StripSingleQuotes(string str)
        {
            int len, start;
        ...
        {
            DecimalX bd;
            Assert.IsTrue(DecimalX.TryParse("", out bd) == false);
        }
        ...
        {
            DecimalX bd;
            Assert.IsTrue(DecimalX.TryParse("0E", out bd) == false);
            Assert.IsTrue(DecimalX.TryParse("0e", out bd) == false);
            Assert.IsTrue(DecimalX.TryParse("0E+", out bd) == false);
            Assert.IsTrue(DecimalX.TryParse("0E-", out bd) == false);
            Assert.IsTrue(DecimalX.TryParse("0e+", out bd) == false);
        ...
        public void ParsingZeroWorks()
        {
            ...
            Assert.IsTrue(DecimalX.TryParse("0", out bd));
        ...
        public void ParsingMultipleZeroWorks()
        {
            ...
            Assert.IsTrue(DecimalX.TryParse("0000", out bd));
            Assert.IsTrue(bd.Coefficient.IsZero());
            Assert.IsTrue(bd.Exponent == 0);
            Assert.IsTrue(bd.Precision == 1);
        }
        ...
        public void ParsingZeroWithDecimalPointWorks()
        {
            ...
            Assert.IsTrue(DecimalX.TryParse("00.00", out bd));
            Assert.IsTrue(bd.Coefficient.IsZero());
        ...
        public void SimpleIntTest(string decString, string intString, int exponent, int precision)
        {
            DecimalX bd;

            Assert.IsTrue(DecimalX.TryParse(decString, out bd));
            Assert.IsTrue(bd.Coefficient == IntegerX.Parse(intString));
            Assert.IsTrue(bd.Exponent == exponent);
            Assert.IsTrue(bd.Precision == (uint)precision);
        }
        #endregion
        ...
        {
            var bi = IntegerX.Parse(biStr);
            var bd = new DecimalX(bi, exp);
            var result = bd.ToScientificString();
        ...
        {
            var c9hu = new MathContext(9, RoundingMode.HalfUp);
        ...
        private void TestDouble(double v, string expectStr, MathContext c)
        {
            var d = DecimalX.Create(v, c);

            var gotStr = d.ToScientificString();

            Assert.IsTrue(gotStr == expectStr);

            if (v != 0.0)
            {
                d = DecimalX.Create(-v, c);
        ...
        public void TestPrecision(string s, int exponent, int precision)
        {
            var bd = new DecimalX(IntegerX.Parse(s), exponent);
            Assert.IsTrue(bd.Precision == (uint)precision);
        }
        #endregion

        #region Constants Test

        [TestCase]
        public void NamedConstantsHaveCorrectValues()
        {
            Assert.IsTrue(DecimalX.Zero.Coefficient == IntegerX.Zero);
            Assert.IsTrue(DecimalX.Zero.Exponent == 0);
            Assert.IsTrue(DecimalX.Zero.Precision == 1);

            Assert.IsTrue(DecimalX.One.Coefficient == IntegerX.One);
            Assert.IsTrue(DecimalX.One.Exponent == 0);
            Assert.IsTrue(DecimalX.One.Precision == 1);

            Assert.IsTrue(DecimalX.Ten.Coefficient == IntegerX.Ten);
            Assert.IsTrue(DecimalX.Ten.Exponent == 0);
            Assert.IsTrue(DecimalX.Ten.Precision == 2);
        }
        ...
        {
            TestBasicRounding("123.456", 0, RoundingMode.HalfUp, "0", 3);
            TestBasicRounding("123.456", 1, RoundingMode.HalfUp, "1", 2);
            TestBasicRounding("123.456", 2, RoundingMode.HalfUp, "12", 1);
            TestBasicRounding("123.456", 3, RoundingMode.HalfUp, "123", 0);
            TestBasicRounding("123.456", 4, RoundingMode.HalfUp, "1235", -1);
            TestBasicRounding("123.456", 5, RoundingMode.HalfUp, "12346", -2);
            TestBasicRounding("123.456", 6, RoundingMode.HalfUp, "123456", -3);
            TestBasicRounding("123.456", 7, RoundingMode.HalfUp, "123456", -3);
        }

        [TestCase]
        public void TestJavaDocRoundingTests()
        {
            TestBasicRounding("5.5", 1, RoundingMode.Up, "6", 0);
            TestBasicRounding("5.5", 1, RoundingMode.Down, "5", 0);
            TestBasicRounding("5.5", 1, RoundingMode.Ceiling, "6", 0);
            TestBasicRounding("5.5", 1, RoundingMode.Floor, "5", 0);
            TestBasicRounding("5.5", 1, RoundingMode.HalfUp, "6", 0);
            TestBasicRounding("5.5", 1, RoundingMode.HalfDown, "5", 0);
            TestBasicRounding("5.5", 1, RoundingMode.HalfEven, "6", 0);

            TestBasicRounding("2.5", 1, RoundingMode.Up, "3", 0);
            TestBasicRounding("2.5", 1, RoundingMode.Down, "2", 0);
            TestBasicRounding("2.5", 1, RoundingMode.Ceiling, "3", 0);
            TestBasicRounding("2.5", 1, RoundingMode.Floor, "2", 0);
            TestBasicRounding("2.5", 1, RoundingMode.HalfUp, "3", 0);
            TestBasicRounding("2.5", 1, RoundingMode.HalfDown, "2", 0);
            TestBasicRounding("2.5", 1, RoundingMode.HalfEven, "2", 0);

            TestBasicRounding("1.6", 1, RoundingMode.Up, "2", 0);
            TestBasicRounding("1.6", 1, RoundingMode.Down, "1", 0);
            TestBasicRounding("1.6", 1, RoundingMode.Ceiling, "2", 0);
            TestBasicRounding("1.6", 1, RoundingMode.Floor, "1", 0);
            TestBasicRounding("1.6", 1, RoundingMode.HalfUp, "2", 0);
            TestBasicRounding("1.6", 1, RoundingMode.HalfDown, "2", 0);
            TestBasicRounding("1.6", 1, RoundingMode.HalfEven, "2", 0);

            TestBasicRounding("1.1", 1, RoundingMode.Up, "2", 0);
            TestBasicRounding("1.1", 1, RoundingMode.Down, "1", 0);
            TestBasicRounding("1.1", 1, RoundingMode.Ceiling, "2", 0);
            TestBasicRounding("1.1", 1, RoundingMode.Floor, "1", 0);
            TestBasicRounding("1.1", 1, RoundingMode.HalfUp, "1", 0);
            TestBasicRounding("1.1", 1, RoundingMode.HalfDown, "1", 0);
            TestBasicRounding("1.1", 1, RoundingMode.HalfEven, "1", 0);

            TestBasicRounding("1.0", 1, RoundingMode.Up, "1", 0);
            TestBasicRounding("1.0", 1, RoundingMode.Down, "1", 0);
            TestBasicRounding("1.0", 1, RoundingMode.Ceiling, "1", 0);
            TestBasicRounding("1.0", 1, RoundingMode.Floor, "1", 0);
            TestBasicRounding("1.0", 1, RoundingMode.HalfUp, "1", 0);
            TestBasicRounding("1.0", 1, RoundingMode.HalfDown, "1", 0);
            TestBasicRounding("1.0", 1, RoundingMode.HalfEven, "1", 0);
            TestBasicRounding("1.0", 1, RoundingMode.Unnecessary, "1", 0);
            ...
            TestBasicRounding("-1.1", 1, RoundingMode.Up, "-2", 0);
            TestBasicRounding("-1.1", 1, RoundingMode.Down, "-1", 0);
            TestBasicRounding("-1.1", 1, RoundingMode.Ceiling, "-1", 0);
            TestBasicRounding("-1.1", 1, RoundingMode.Floor, "-2", 0);
            TestBasicRounding("-1.1", 1, RoundingMode.HalfUp, "-1", 0);
            TestBasicRounding("-1.1", 1, RoundingMode.HalfDown, "-1", 0);
            TestBasicRounding("-1.1", 1, RoundingMode.HalfEven, "-1", 0);
            ...
            TestBasicRounding("-1.6", 1, RoundingMode.Up, "-2", 0);
            TestBasicRounding("-1.6", 1, RoundingMode.Down, "-1", 0);
            TestBasicRounding("-1.6", 1, RoundingMode.Ceiling, "-1", 0);
            TestBasicRounding("-1.6", 1, RoundingMode.Floor, "-2", 0);
            TestBasicRounding("-1.6", 1, RoundingMode.HalfUp, "-2", 0);
            TestBasicRounding("-1.6", 1, RoundingMode.HalfDown, "-2", 0);
            TestBasicRounding("-1.6", 1, RoundingMode.HalfEven, "-2", 0);
            ...
            TestBasicRounding("-2.5", 1, RoundingMode.Up, "-3", 0);
            TestBasicRounding("-2.5", 1, RoundingMode.Down, "-2", 0);
            TestBasicRounding("-2.5", 1, RoundingMode.Ceiling, "-2", 0);
            TestBasicRounding("-2.5", 1, RoundingMode.Floor, "-3", 0);
            TestBasicRounding("-2.5", 1, RoundingMode.HalfUp, "-3", 0);
            TestBasicRounding("-2.5", 1, RoundingMode.HalfDown, "-2", 0);
            TestBasicRounding("-2.5", 1, RoundingMode.HalfEven, "-2", 0);
            ...
            TestBasicRounding("-5.5", 1, RoundingMode.Up, "-6", 0);
            TestBasicRounding("-5.5", 1, RoundingMode.Down, "-5", 0);
            TestBasicRounding("-5.5", 1, RoundingMode.Ceiling, "-5", 0);
            TestBasicRounding("-5.5", 1, RoundingMode.Floor, "-6", 0);
            TestBasicRounding("-5.5", 1, RoundingMode.HalfUp, "-6", 0);
            TestBasicRounding("-5.5", 1, RoundingMode.HalfDown, "-5", 0);
            TestBasicRounding("-5.5", 1, RoundingMode.HalfEven, "-6", 0);

            TestBasicRounding("0.19", 3, RoundingMode.Floor, "19", -2);
            TestBasicRounding(".190909", 3, RoundingMode.Floor, "190", -3);
            TestBasicRounding("999.9", 3, RoundingMode.Up, "100", 1);
        }

        public void TestBasicRounding(string bdStr, uint precision, RoundingMode mode, string biStr, int exponent)
        {
            var mc = new MathContext(precision, mode);
            var bd = DecimalX.Parse(bdStr);
            var br = DecimalX.Round(bd, mc);
            Assert.IsTrue(br.Coefficient == IntegerX.Parse(biStr));
        ...
        public void TQ(string test, RoundingMode mode)
        {
            string lhsStr, rhsStr, resultStr;
            GetThreeArgs(test, out lhsStr, out rhsStr, out resultStr);
            TestQuantize(lhsStr, rhsStr, mode, resultStr);
        }

        public void TestQuantize(string lhsStr, string rhsStr, RoundingMode m, string shouldStr)
        {
            var lhs = DecimalX.Parse(lhsStr);
            var rhs = DecimalX.Parse(rhsStr);
            var result = DecimalX.Quantize(lhs, rhs, m);
            var resultStr = result.ToScientificString();
        ...
        {
            var c9 = new MathContext(9, RoundingMode.HalfUp);
            var c7 = new MathContext(7, RoundingMode.HalfUp);
            var c6 = new MathContext(6, RoundingMode.HalfUp);
            var c3 = new MathContext(3, RoundingMode.HalfUp);
        ...
        public void TAbs(string test, MathContext c)
        {
            string argStr, resultStr;
            GetTwoArgs(test, out argStr, out resultStr);
            TestAbs(argStr, c, resultStr);
        }

        public void TestAbs(string argStr, MathContext c, string shouldStr)
        {
            var arg = DecimalX.Parse(argStr);
            var result = DecimalX.Abs(arg, c);
            var resultStr = result.ToScientificString();
            Assert.IsTrue(resultStr == shouldStr);
        }
        ...
        public void SpecAdd()
        {
            var c6hu = new MathContext(6, RoundingMode.HalfUp);
            var c9hu = new MathContext(9, RoundingMode.HalfUp);
            var c15hu = new MathContext(15, RoundingMode.HalfUp);
            ...
            var c9he = new MathContext(9, RoundingMode.HalfEven);
            ...
            var c9d = new MathContext(9, RoundingMode.Down);
            ...
            // precision: 3
            // rounding: half_up
            var c3hu = new MathContext(3, RoundingMode.HalfUp);
            ...
            var c3hd = new MathContext(3, RoundingMode.HalfDown);
          ...
          //   minexponent: -92
            ...
            var c6hd = new MathContext(6, RoundingMode.HalfDown);
            var c6he = new MathContext(6, RoundingMode.HalfEven);
            var c7hu = new MathContext(7, RoundingMode.HalfUp);
            var c10hd = new MathContext(10, RoundingMode.HalfDown);
            var c10hu = new MathContext(10, RoundingMode.HalfUp);
            var c10he = new MathContext(10, RoundingMode.HalfEven);
        ...
        public void TAdd(string test, MathContext c)
        {
        ...
        public void TestAddition(string arg1Str, string arg2Str, MathContext c, string resultStr)
        {
            var arg1 = DecimalX.Parse(arg1Str);
            var arg2 = DecimalX.Parse(arg2Str);
            var val = arg1.Add(arg2, c);
            var valStr = val.ToScientificString();
        ...
        {
            MathContext c9hu, c6hu, c3hu, c12hu, c15hu, c1hu, c2hu, c4hu, c5hu, c7hu, c8hu, c9d, c34hu, c9he;

            c9hu = new MathContext(9, RoundingMode.HalfUp);
            // version: 2.59
            //
            // extended:    1
            // precision:   9
            ...
            // precision: 6
            c6hu = new MathContext(6, RoundingMode.HalfUp);
            ...
            c3hu = new MathContext(3, RoundingMode.HalfUp);
            c12hu = new MathContext(12, RoundingMode.HalfUp);
            c15hu = new MathContext(15, RoundingMode.HalfUp);
            ...
            c1hu = new MathContext(1, RoundingMode.HalfUp);
            c2hu = new MathContext(2, RoundingMode.HalfUp);
            c4hu = new MathContext(4, RoundingMode.HalfUp);
            c5hu = new MathContext(5, RoundingMode.HalfUp);
            c7hu = new MathContext(7, RoundingMode.HalfUp);
            c8hu = new MathContext(8, RoundingMode.HalfUp);
            ...
            // rounding: half_even
            c9he = new MathContext(9, RoundingMode.HalfEven);
            ...
            // rounding: down
            c9d = new MathContext(9, RoundingMode.Down);
            ...
            c34hu = new MathContext(34, RoundingMode.HalfUp);
        ...
        public void TSub(string test, MathContext c)
        {
            string arg1Str, arg2Str, resultStr;
            GetThreeArgs(test, out arg1Str, out arg2Str, out resultStr);
        ...
        public void TestSubtraction(string arg1Str, string arg2Str, MathContext c, string resultStr)
        {
            var arg1 = DecimalX.Parse(arg1Str);
            var arg2 = DecimalX.Parse(arg2Str);
            var val = arg1.Subtract(arg2, c);
            var valStr = val.ToScientificString();
            Assert.IsTrue(valStr == resultStr);
        }
        #endregion
        ...
        {
            MathContext c6hu, c7hu, c9hu, c15hu, c30hu, c33hu, c8hu;
            ...
            c9hu = new MathContext(9, RoundingMode.HalfUp);
            c15hu = new MathContext(15, RoundingMode.HalfUp);
            c30hu = new MathContext(30, RoundingMode.HalfUp);
            c33hu = new MathContext(33, RoundingMode.HalfUp);
            ...
            // precision: 33
            TMul("mulx246 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801193369671916511992830 Inexact Rounded", new MathContext(33, RoundingMode.HalfUp));
            // precision: 32
            TMul("mulx247 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080119336967191651199283  Inexact Rounded", new MathContext(32, RoundingMode.HalfUp));
            // precision: 31
            TMul("mulx248 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908011933696719165119928   Inexact Rounded", new MathContext(31, RoundingMode.HalfUp));
            // precision: 30
            TMul("mulx249 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801193369671916511993    Inexact Rounded", new MathContext(30, RoundingMode.HalfUp));
            // precision: 29
            TMul("mulx250 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080119336967191651199     Inexact Rounded", new MathContext(29, RoundingMode.HalfUp));
            // precision: 28
            TMul("mulx251 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908011933696719165120      Inexact Rounded", new MathContext(28, RoundingMode.HalfUp));
            // precision: 27
            TMul("mulx252 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801193369671916512       Inexact Rounded", new MathContext(27, RoundingMode.HalfUp));
            // precision: 26
            TMul("mulx253 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080119336967191651        Inexact Rounded", new MathContext(26, RoundingMode.HalfUp));
            // precision: 25
            TMul("mulx254 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908011933696719165         Inexact Rounded", new MathContext(25, RoundingMode.HalfUp));
            // precision: 24
            TMul("mulx255 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801193369671917          Inexact Rounded", new MathContext(24, RoundingMode.HalfUp));
            // precision: 23
            TMul("mulx256 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080119336967192           Inexact Rounded", new MathContext(23, RoundingMode.HalfUp));
            // precision: 22
            TMul("mulx257 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908011933696719            Inexact Rounded", new MathContext(22, RoundingMode.HalfUp));
            // precision: 21
            TMul("mulx258 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801193369672             Inexact Rounded", new MathContext(21, RoundingMode.HalfUp));
            // precision: 20
            TMul("mulx259 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080119336967              Inexact Rounded", new MathContext(20, RoundingMode.HalfUp));
            // precision: 19
            TMul("mulx260 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908011933697               Inexact Rounded", new MathContext(19, RoundingMode.HalfUp));
            // precision: 18
            TMul("mulx261 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801193370                Inexact Rounded", new MathContext(18, RoundingMode.HalfUp));
            // precision: 17
            TMul("mulx262 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080119337                 Inexact Rounded",
              new MathContext(17, RoundingMode.HalfUp));
            // precision: 16
            TMul("mulx263 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908011934                  Inexact Rounded", new MathContext(16, RoundingMode.HalfUp));
            // precision: 15
            TMul("mulx264 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801193                   Inexact Rounded", new MathContext(15, RoundingMode.HalfUp));
            // precision: 14
            TMul("mulx265 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080119                    Inexact Rounded", new MathContext(14, RoundingMode.HalfUp));
            // precision: 13
            TMul("mulx266 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908012                     Inexact Rounded", new MathContext(13, RoundingMode.HalfUp));
            // precision: 12
            TMul("mulx267 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.290801                      Inexact Rounded", new MathContext(12, RoundingMode.HalfUp));
            // precision: 11
            TMul("mulx268 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29080                       Inexact Rounded", new MathContext(11, RoundingMode.HalfUp));
            // precision: 10
            TMul("mulx269 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.2908                        Inexact Rounded", new MathContext(10, RoundingMode.HalfUp));
            // precision:  9
            TMul("mulx270 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.291                         Inexact Rounded", new MathContext(9, RoundingMode.HalfUp));
            // precision:  8
            TMul("mulx271 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.29                          Inexact Rounded", new MathContext(8, RoundingMode.HalfUp));
            // precision:  7
            TMul("mulx272 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433.3                           Inexact Rounded", new MathContext(7, RoundingMode.HalfUp));
            // precision:  6
            TMul("mulx273 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 145433                            Inexact Rounded", new MathContext(6, RoundingMode.HalfUp));
            // precision:  5
            TMul("mulx274 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 1.4543E+5                         Inexact Rounded", new MathContext(5, RoundingMode.HalfUp));
            // precision:  4
            TMul("mulx275 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 1.454E+5                         Inexact Rounded", new MathContext(4, RoundingMode.HalfUp));
            // precision:  3
            TMul("mulx276 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 1.45E+5                         Inexact Rounded", new MathContext(3, RoundingMode.HalfUp));
            // precision:  2
            TMul("mulx277 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 1.5E+5                         Inexact Rounded", new MathContext(2, RoundingMode.HalfUp));
            // precision:  1
            TMul("mulx278 multiply 30269.587755640502150977251770554 4.8046009735990873395936309640543 -> 1E+5                          Inexact Rounded", new MathContext(1, RoundingMode.HalfUp));
            ...
            // -- fastpath breakers
            // precision:   29
            TMul("mulx330 multiply 1.491824697641270317824852952837224 1.105170918075647624811707826490246514675628614562883537345747603 -> 1.6487212707001281468486507878 Inexact Rounded", new MathContext(29, RoundingMode.HalfUp));
            // precision:   55
            TMul("mulx331 multiply 0.8958341352965282506768545828765117803873717284891040428 0.8958341352965282506768545828765117803873717284891040428 -> 0.8025187979624784829842553829934069955890983696752228299 Inexact Rounded", new MathContext(55, RoundingMode.HalfUp));
            ...
            // maxExponent: 92
            // minexponent: -92
            // TMul("mulx504  multiply  0E-60 1000E-60  -> 0E-98 Clamped", new MathContext(7, RoundingMode.HalfUp));
            // TMul("mulx505  multiply  100E+60 0E+60   -> 0E+92 Clamped", new MathContext(7, RoundingMode.HalfUp));
        ...
        public void TMul(string test, MathContext c)
        {
            string arg1Str, arg2Str, resultStr;
            GetThreeArgs(test, out arg1Str, out arg2Str, out resultStr);
        ...
        public void TestMultiply(string arg1Str, string arg2Str, MathContext c, string resultStr)
        {
            var arg1 = DecimalX.Parse(arg1Str);
            var arg2 = DecimalX.Parse(arg2Str);
            var val = arg1.Multiply(arg2, c);
            var valStr = val.ToScientificString();
            Assert.IsTrue(valStr == resultStr);
        }
        ...
        public void TestSpecDivide()
        {
            ...
            var c8hu = new MathContext(8, RoundingMode.HalfUp);
            ...
            // -- +ve exponent
            // precision: 5
            var c5hu = new MathContext(5, RoundingMode.HalfUp);
            ...
            // -- long operand triangle
            // precision: 33
            TDiv("divx610 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.8131097703792 Inexact Rounded", new MathContext(33, RoundingMode.HalfUp));
            // precision: 32
            TDiv("divx611 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.813109770379  Inexact Rounded", new MathContext(32, RoundingMode.HalfUp));
            // precision: 31
            TDiv("divx612 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.81310977038   Inexact Rounded", new MathContext(31, RoundingMode.HalfUp));
            // precision: 30
            TDiv("divx613 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.8131097704    Inexact Rounded", new MathContext(30, RoundingMode.HalfUp));
            // precision: 29
            TDiv("divx614 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.813109770     Inexact Rounded", new MathContext(29, RoundingMode.HalfUp));
            // precision: 28
            TDiv("divx615 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.81310977      Inexact Rounded", new MathContext(28, RoundingMode.HalfUp));
            // precision: 27
            TDiv("divx616 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.8131098       Inexact Rounded", new MathContext(27, RoundingMode.HalfUp));
            // precision: 26
            TDiv("divx617 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.813110        Inexact Rounded", new MathContext(26, RoundingMode.HalfUp));
            // precision: 25
            TDiv("divx618 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.81311         Inexact Rounded", new MathContext(25, RoundingMode.HalfUp));
            // precision: 24
            TDiv("divx619 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.8131          Inexact Rounded", new MathContext(24, RoundingMode.HalfUp));
            // precision: 23
            TDiv("divx620 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.813           Inexact Rounded", new MathContext(23, RoundingMode.HalfUp));
            // precision: 22
            TDiv("divx621 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.81            Inexact Rounded", new MathContext(22, RoundingMode.HalfUp));
            // precision: 21
            TDiv("divx622 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817797.8             Inexact Rounded", new MathContext(21, RoundingMode.HalfUp));
            // precision: 20
            TDiv("divx623 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -41011408883796817798               Inexact Rounded", new MathContext(20, RoundingMode.HalfUp));
            // precision: 19
            TDiv("divx624 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.101140888379681780E+19         Inexact Rounded", new MathContext(19, RoundingMode.HalfUp));
            // precision: 18
            TDiv("divx625 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.10114088837968178E+19         Inexact Rounded", new MathContext(18, RoundingMode.HalfUp));
            // precision: 17
            TDiv("divx626 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.1011408883796818E+19         Inexact Rounded", new MathContext(17, RoundingMode.HalfUp));
            // precision: 16
            TDiv("divx627 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.101140888379682E+19         Inexact Rounded", new MathContext(16, RoundingMode.HalfUp));
            // precision: 15
            TDiv("divx628 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.10114088837968E+19         Inexact Rounded", new MathContext(15, RoundingMode.HalfUp));
            // precision: 14
            TDiv("divx629 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.1011408883797E+19         Inexact Rounded", new MathContext(14, RoundingMode.HalfUp));
            // precision: 13
            TDiv("divx630 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.101140888380E+19         Inexact Rounded", new MathContext(13, RoundingMode.HalfUp));
            // precision: 12
            TDiv("divx631 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.10114088838E+19         Inexact Rounded", new MathContext(12, RoundingMode.HalfUp));
            // precision: 11
            TDiv("divx632 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.1011408884E+19         Inexact Rounded", new MathContext(11, RoundingMode.HalfUp));
            // precision: 10
            TDiv("divx633 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.101140888E+19         Inexact Rounded", new MathContext(10, RoundingMode.HalfUp));
            // precision:  9
            TDiv("divx634 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.10114089E+19         Inexact Rounded", new MathContext(9, RoundingMode.HalfUp));
            // precision:  8
            TDiv("divx635 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.1011409E+19         Inexact Rounded", new MathContext(8, RoundingMode.HalfUp));
            // precision:  7
            TDiv("divx636 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.101141E+19         Inexact Rounded", new MathContext(7, RoundingMode.HalfUp));
            // precision:  6
            TDiv("divx637 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.10114E+19         Inexact Rounded", new MathContext(6, RoundingMode.HalfUp));
            // precision:  5
            TDiv("divx638 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.1011E+19         Inexact Rounded", new MathContext(5, RoundingMode.HalfUp));
            // precision:  4
            TDiv("divx639 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.101E+19         Inexact Rounded", new MathContext(4, RoundingMode.HalfUp));
            // precision:  3
            TDiv("divx640 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.10E+19         Inexact Rounded", new MathContext(3, RoundingMode.HalfUp));
            // precision:  2
            TDiv("divx641 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4.1E+19         Inexact Rounded", new MathContext(2, RoundingMode.HalfUp));
            // precision:  1
            TDiv("divx642 divide -3374988581607586061255542201048 82293895124.90045271504836568681 -> -4E+19         Inexact Rounded", new MathContext(1, RoundingMode.HalfUp));
            ...
            // maxExponent: 384
            // minExponent: -383
            var c16hu = new MathContext(16, RoundingMode.HalfUp);
            ...
            var c7hd = new MathContext(7, RoundingMode.HalfDown);
            var c7he = new MathContext(7, RoundingMode.HalfEven);
        ...
        {
            var c0u = new MathContext(0, RoundingMode.Unnecessary);
        ...
        public void TDivEx(string test, MathContext c)
        {
            string arg1Str, arg2Str, resultStr;
        ...
        public void TDiv(string test, MathContext c)
        {
            string arg1Str, arg2Str, resultStr;
            GetThreeArgs(test, out arg1Str, out arg2Str, out resultStr);
        ...
        public void TestDivide(string arg1Str, string arg2Str, MathContext c, string resultStr)
        {
            var arg1 = DecimalX.Parse(arg1Str);
            var arg2 = DecimalX.Parse(arg2Str);
            var val = arg1.Divide(arg2, c);
            var valStr = val.ToScientificString();
            Assert.IsTrue(valStr == resultStr);
        }
        ...
        public void TDivI(string test, MathContext c)
        {
            string arg1Str, arg2Str, resultStr;
        ...
        public void TestDivideInt(string arg1Str, string arg2Str, MathContext c, string resultStr)
        {
            var arg1 = DecimalX.Parse(arg1Str);
            var arg2 = DecimalX.Parse(arg2Str);
            var val = arg1.DivideInteger(arg2, c);
            var valStr = val.ToScientificString();
            Assert.IsTrue(valStr == resultStr);
        }

        public void TDivIEx(string test, MathContext c)
        {
            string arg1Str, arg2Str, resultStr;
            DecimalX arg1, arg2;
            ...
            {
                GetThreeArgs(test, out arg1Str, out arg2Str, out resultStr);
                arg1 = DecimalX.Parse(arg1Str);
                arg2 = DecimalX.Parse(arg2Str);
        ...
        public void PowerTestsFromSpec()
        {
            var c9he = new MathContext(9, RoundingMode.HalfEven);
            var c10he = new MathContext(10, RoundingMode.HalfEven);
            var c16he = new MathContext(16, RoundingMode.HalfEven);
            // extended:    1
            // precision:   16
            // rounding:    half_even
            // maxExponent: 384
        ...
        public void TPow(string test, MathContext c)
        {
            string arg1Str, arg2Str, resultStr;
        ...
        public void TestPower(string arg1Str, string arg2Str, MathContext c, string resultStr)
        {
            var arg1 = DecimalX.Parse(arg1Str);
            var arg2 = int.Parse(arg2Str);
            var val = arg1.Power(arg2, c);
            var valStr = val.ToScientificString();
            Assert.IsTrue(valStr == resultStr);
        }
        ...
        public void TestMoveLeft(string strVal, int n, string expectedString)
        {
            var d = DecimalX.Parse(strVal);
            var m = d.MovePointLeft(n);
            var resultStr = m.ToScientificString();
            Assert.IsTrue(resultStr == expectedString);
        }

        public void TestMoveRight(string strVal, int  n, string expectedString)
        {
            var d = DecimalX.Parse(strVal);
            var m = d.MovePointRight(n);
            var resultStr = m.ToScientificString();
            Assert.IsTrue(resultStr == expectedString);
        }
